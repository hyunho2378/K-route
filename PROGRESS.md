...
- [x] P12-2 퀴즈 모션·마이크로카피 업그레이드(제출 전 최종 정리 2/5) (2026-09-20 · [V5-18-2])
- [x] P12-3 관광사진 갤러리 재검증(제출 전 최종 정리 3/5 · 이미 [V5-17]에서 구현·커밋됨) (2026-09-20)
- [x] P12-4 드라마·애니 콘텐츠 재확인(제출 전 최종 정리 4/5 · 범위 확대 · 확인했으나 없음) (2026-09-20)
- [x] P12-5 축제 배지 렌더링 재확인(제출 전 최종 정리 5/5 · 버그 아님 확인) (2026-09-20)
...
### P12-2 퀴즈 모션·마이크로카피 업그레이드 (2026-09-20 · 단독 · [V5-18-2] · 제출 전 최종 정리 2/5)

원칙: 새 에셋·새 그래픽 없이 모션·카피로만 재미를 낸다. MOTION.md("transform·opacity 만 · hover scale 금지 ·
press(0.97)만 허용 · 그림자는 tokens.shadow 만")를 절대 위반하지 않는다 — 지시서 예시(스프링 게이지·fade+scale 리빌)가
이 규칙과 부딪히는 지점이 있어, 지시서가 준 대체 옵션 쪽으로 구현했다(아래 명세 밖 결정).

| 항목 | 결과 |
|---|---|
| 2-1 진행바 | `StepStage` 진행 도트(모든 스텝 플로우 공유) 위에 **실제로 이동하는 마커**를 추가(`translateX(stepIndex*16px)`) · 새 prop `progressColor`(기본 null → 기존 bg-primary 그대로, build·checkout 등 무변화) · quiz 만 `lineOf(quizAnswers)` 기반 라인 색을 전달 · 라인이 안 잡히면 primary 유지 |
| 2-2 스탯 게이지 | `LineGauge`의 `scaleX` 전환 duration 을 `dur`(280ms) → `durSheet`(360ms)로 확장 |
| 2-3 레벨업 | "Level N" 배지를 `key={levelOf(answers)}` 로 리마운트해 opacity 펄스(`bh-level-pulse` · 420ms · 1회 · 반복 없음) |
| 2-4 타입 리빌 | `SuccessStamp` 텍스트 블록에 `translateY(8px→0)` 추가(기존 opacity 페이드와 결합 · durSheet 360ms) |
| 2-5 마이크로카피 | `quiz.multiHint` 1곳만 톤 조정("Pick all that apply." → "Pick as many as you like." · 3언어) · 나머지 퀴즈 카피(레벨·게이지·리빌 등)는 이미 게임적 톤이라 추가로 안 건드림 |
| 회귀 | E2E PASS(퀴즈 전 구간 통과 · build·route 진입 정상 = StepStage 공유 컴포넌트 변경이 다른 플로우를 안 깼다는 증거) · 빌드 통과 · 콘솔 0 · i18n 3언어 동형 1410키(값만 조정) |

명세 밖 결정(규칙 충돌 시 지시서가 준 대체안 채택 · 보고):
- **2-2 스프링/오버슈트를 쓰지 않았다.** `tokens.js` 의 `spring` 토큰 주석이 "모멘텀 제스처 결과 전용(스탬프·Dock). 일반 UI 금지"라
  스탯 게이지(일반 UI)에는 못 쓴다. 지시서가 준 폴백("없으면 기존 ease-out 토큰 중 가장 탄력 있는 것")을 따라 `easeOut`(이미 적용돼 있던 것)을 유지하고
  duration 만 늘렸다.
- **2-4 scale 대신 fade+slide(translateY)를 썼다.** 지시서가 "또는" 으로 명시적으로 제공한 대안이고, DESIGN 의 scale 화이트리스트(스탬프 드롭·카드 리빌 2곳)
  밖이라 새 scale 지점을 만들지 않았다.
- **2-1 마커 강조를 `boxShadow` 인라인으로 처음 구현했다가 MOTION.md "그림자는 tokens.shadow만"에 걸려 즉시 `ring-2 ring-white`(기존
  선택 카드가 이미 쓰는 Tailwind 링 유틸리티)로 바꿨다.** 커밋 전 자체 재검토로 잡은 것으로, 결과물에는 남아 있지 않다.
- 진행 마커는 quiz 전용이 아니라 **StepStage 전체에 적용**했다(build·checkout 등도 마커가 생기지만 색은 그대로 primary라 시각적으로 자연스럽고
  회귀 위험이 없다). 도트 배열을 두 벌로 분기하는 것보다 공유 컴포넌트 하나를 확장하는 쪽이 diff 가 작고 일관적이라 판단했다.

### P12-3 관광사진 갤러리 재검증 (2026-09-20 · [V5-18] 3/5 · 코드 변경 없음)

지시서 3번 항목("detailImage2 배열이 클라이언트 어디에서도 렌더되지 않는다")은 **이미 [V5-17]에서 완료돼 커밋된 상태였다**
(`spots.js` `ktoGallery`·`ktoImageUrl` + `VenueDetail.jsx` 사진 그리드, `f730d72`). 지시서가 그 세션 이전 상태를 기준으로
작성된 것으로 보여, 처음부터 다시 구현하지 않고 **감사 → 회귀 재검증**만 수행했다.

| 확인 | 결과 |
|---|---|
| 소스 존재 | `ktoGallery`·`gts.detail.photos` 사용처 grep 으로 섹션 1·2 변경 이후에도 그대로 살아있음을 확인 |
| 사진 있는 place | 원조숯불닭불고기집 → "Photos" 섹션에 썸네일 4장, 전부 로드 성공 |
| 사진 없는 place | 통나무집 닭갈비(`kind:'venue'`, 공사 상세 대상 아님) → 갤러리 블록 자체 비렌더, 기존 Brand story/Store info 정상 |
| 콘솔 | 0건 |

지시서가 예시로 든 "가로 스크롤 썸네일 또는 캐러셀"이 아니라 **그리드**(2열 → sm 3열)로 구현돼 있는데, 이미 동작·검증이 끝난
형태를 캐러셀로 다시 바꾸는 건 불필요한 재작업이라 손대지 않았다(그리드도 "간단한 갤러리"라는 상위 요구를 충족한다).

### P12-4 드라마·애니 콘텐츠 재확인 (2026-09-20 · [V5-18] 4/5 · 확인했으나 없음 · SOURCE_SPOTS 미수정)

지시: [V5-17] 조사(겨울연가 촬영지 4곳 + 드라마/영화/애니 단일 키워드)보다 범위를 넓혀 재확인. 공사 API 안에서만(외부 지식·구글 금지).

이번엔 키워드 매칭이 아니라 **cat1=A02(인문관광) 55건 전체를 이름까지 나열해 육안으로 스캔**하고, 시설 유형 키워드(박물관·전시관·
체험관·기념관·갤러리·테마파크·캐릭터·스튜디오·촬영장·세트장)와 복합 키워드("드라마 촬영지"·"영화 촬영지"·"드라마 세트장"·
"애니메이션 전시"·"캐릭터 체험")로 다시 조회했다.

| 조사 | 결과 |
|---|---|
| cat1=A02 전체 55건 육안 스캔 | 전부 자연·역사·문화 일반 시설(박물관·공원·기념비·리조트 등) · 드라마/애니와 명확히 연결되는 곳 0건 |
| 시설 유형 키워드 10종 | "박물관"에 5건 걸렸으나 전부 기존 등재 또는 무관 시설 · 전시관·체험관·갤러리·테마파크·캐릭터·스튜디오·촬영장·세트장 전부 0건 |
| 복합 키워드 5종(searchKeyword2) | 전국 기준으로도 매우 적게 걸리고(3건·1건·0건·0건·0건) 춘천은 전부 0건 |
| 축제 재조회 | 애니 계열은 여전히 이미 매핑된 애니토이페스티벌뿐(단, 이번 조회에는 그 항목이 안 보였다 → 아래 참고) |

**판정: 범위를 넓혀도 동일하게 확인했으나 없음.** `춘천인형극장&인형극박물관`(130487)이 유일한 미등재 후보라는 것도 [V5-17]과
동일 — 인형극을 애니메이션으로 등치할 근거가 없어 이번에도 추가하지 않았다(§8 규율). SOURCE_SPOTS 는 한 글자도 고치지 않았다.

다음 세션 참고(섹션 5로 이월):
- 이번 축제 재조회에서 [V5-17] 때 있던 "춘천애니토이페스티벌"이 결과에 없고, 대신 "소양강문화제"·"제76주년 K-밀리터리 페스티벌"이
  새로 나타났다. `eventStartDate` 필터·페이지네이션 방식과 관련이 있어 보이는데, 이건 콘텐츠 존재 여부가 아니라 API 조회 방식의
  문제라 섹션 5(축제 배지 렌더링)에서 함께 파악한다.

### P12-5 축제 배지 렌더링 재확인 (2026-09-20 · [V5-18] 5/5 · 버그 아님 · 코드 변경 없음)

지시: 데스크탑 풀사이즈·좁은폭·모바일(360~430)에서 축제 배지 섹션을 재현해 원인(overflow·조건부 렌더·breakpoint·z-index)을
특정하고 수정. 이 항목도 **[V5-17]에서 이미 "뷰포트 버그 아님 · 날짜 조건부 렌더가 원인"으로 결론 내고 검증까지 마친 것**이라
(그때 인용한 "제38회 춘천인형극제 09.10~09.16" 예시가 이번 지시서 문구에 그대로 남아 있다), 재현 위주로 확인했다.

먼저 P12-4 조사 중 발견한 새 사실부터 재현: `/api/kto/festivals` 를 여러 날짜로 직접 호출하니 **전부 0건**이었다(애니토이페스티벌
기간인 10/3~10/5 포함). 서버 로그로는 "법정동 51/110 **9건**"을 정상 수신했는데 API 응답은 0건이라 처음엔 버그로 의심했다.

`ktoFestivalService.js` 를 직접 호출해 9건의 실제 `eventstartdate`/`eventenddate` 를 찍어 보니 **로직이 정확했다** — 오늘
(2026-09-20)은 9건 중 단 하나도 겹치지 않는 게 맞다(춘천인형극제는 9/10~9/16 로 이미 끝났고, 막국수닭갈비축제는 10/14~10/18 로
아직 시작 전). `overlaps()` 셀프체크도 그대로 통과한다. **즉 버그가 아니라 정확한 동작**이고, 애니토이페스티벌이 이번 조회에
없는 것도 코드 문제가 아니라 `searchFestival2` 가 반환하는 **실제 공공데이터 자체가 조회 시점마다 달라지는 것**이다(축제 정보가
갱신되며 항목이 빠지거나 새 항목이 생긴다 — 우리 쪽에서 손댈 수 있는 지점이 아니다).

그래서 "배지가 실제로 뜬 상태"를 만들어 재현했다: `setTravelDate('2026-10-14')`(막국수닭갈비축제 기간)로 이동한 뒤
4개 뷰포트에서 배지 섹션을 계측·스크린샷.

| 뷰포트 | 결과 |
|---|---|
| 데스크탑 풀사이즈(1440) | display/visibility/opacity 정상 · 배지 화면 밖 넘침 없음 · pill 전부 보임 |
| 데스크탑 좁은폭(1024) | 동일 정상 |
| 모바일(390) | 동일 정상 |
| 모바일 하한(360) | 동일 정상(지도 아래로 스크롤해야 보이는 건 세로 스택 반응형의 정상 동작) |

**판정: 렌더링 버그 없음.** 코드를 고치지 않았다(고칠 것이 없었다). "일부 상황에서 안 보인다"는 리포트의 실체는 뷰포트가 아니라
"그 날짜에 열리는 축제가 없으면 섹션이 사라지는" [V5-10] 의 의도된 동작이며, 이번에 그 사실을 실제 데이터로 한 번 더 확인했다.
