...
// [P3-A] 상세 패널 odii 해설 · 장소명 완전일치 테마만(ktoAudioService · 좌표 판정 없음) · 없거나 실패면 null(클라 오디오 블록 비렌더)
const audioOf = (item, lang) =>
  getAudioGuide(item.name, lang).catch((e) => {
...
//   [V5-3] audio = Odii 해설(themeSearchList 장소명 완전일치 → storyBasedList 원문 · 없으면 null)
